...
const KeepAlive = {
  ...
  setup(props, { slots }) {
    ...
    // 对于KeepAlive组件来说，它的实例上存在特殊的 keepAliveCtx 对象，该对象由渲染器注入，该对象会暴露渲染器
    // 的一些内部方法，其中 move 函数用来将一段 DOM 移动到另一个容器中
...
const Teleport = {
  ...
  process(n1, n2, container, anchor, internals) {
    ...
    if (!n1) {
      ...
      if (n2.props.to !== n2.props.to) {
        // 获取新的容器
        const newTarget =
          typeof n2.props.tp === "string"
            ? document.querySelector(n2.props.to)
            : n2.props.to;
...
function createRenderer(options) {
  ...
  const patch = (n1, n2, container, anchor) => {
    ...
    if (typeof type === "string") {
      ...
      // 组件选项中如果存在 __isTeleport 标识，则它是 Teleport 组件，调用 Teleport 组件选项中的 process 函数
      // 将控制权交接出去， 传递给 process 函数的第五个参数是渲染器的一些内部方法
